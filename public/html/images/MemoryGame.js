...
var MemoryGame = function (evt) {
    ...
    var lvlNum      = 0,
        ...
        start       = function () {
			...
			lvlCtrls.childNodes[1].className = 'selected';
			var i =1;
			while(i<16){ 
				if(i > currentLvlValue)
					lvlCtrls.childNodes[i].className = lvlCtrls.childNodes[i].className == 'selected' ? 'selected' : 'disable-btn'
				//lvlCtrls.childNodes[i].className = 'disable-btn'
				i = i+2;
				}
            currentLvl.onwin = function (clicks, prc) {
                info.innerHTML = 'You\'e found all matches in <strong>' + clicks + '</strong> clicks with <strong>' + prc + '%</strong> efficiency';
				
				// TODO: Send current level.
				
				lvlCtrls.childNodes[currentLvlValue].className = '';
				lvlCtrls.childNodes[currentLvlValue+2].classList.remove("disable-btn");
				lvlCtrls.childNodes[currentLvlValue+2].className = 'selected';
				currentLvlValue = currentLvlValue+2;
   // $('#myModal1').modal;
            };
    ...
    evt.attach('mousedown', lvlCtrls, function (e, src) {
		...
		if(lvlCtrls.childNodes[1] != src.parentNode)
			lvlCtrls.childNodes[1].className = '';
    });
};
